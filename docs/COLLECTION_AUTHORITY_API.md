...
Both clients retry only transport failures and HTTP 408/425/429/5xx, honor `Retry-After`, check contract versions, and reject a snapshot that is not exactly seven lanes/689 ProductRefs. Authentication, schema, identity, quantity, and revision errors are not retried.
