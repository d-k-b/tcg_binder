...
assert.strictEqual(productCount, 689);
assert.strictEqual(wrapperCount, 378);
assert.strictEqual(productCount + wrapperCount, 1067, 'the scanner must constrain matching to every canonical product and reviewed wrapper ID');
...
console.log('product-identification dashboard tests: 1067 catalog candidates, camera/upload UI, exact bridge validation, explicit-only mutation, image re-encoding, and wrapper quantity migration passing');
