(function initPricingClient(root, factory) {
  ...
  function createClient(options) {
    ...
    return {
      ...
      decorateCollectionPage: (snapshot, requestOptions) => send("pricing.page.decorateCollection", { snapshot, options: requestOptions || {} }),
      syncMonitorCollection: (subscription) => send("pricing.monitor.syncCollection", { subscription }),
      monitorStatus: () => send("pricing.monitor.status"),
      runMonitor: () => send("pricing.monitor.run", { options: { userInitiated: true } }),
