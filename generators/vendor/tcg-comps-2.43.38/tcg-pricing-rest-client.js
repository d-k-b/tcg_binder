(function initTCGPricingRestClient(root, factory) {
  ...
  function createClient({ baseUrl, accessToken, timeoutMs = 60000, fetchImpl = root.fetch } = {}) {
    ...
    return {
      async priceProduct(product, options = {}) {
        ...
        let response;
        try {
          response = await fetchImpl(endpoint + "/v1/price", {
            method: "POST",
            headers: { Accept: "application/json", Authorization: "Bearer " + token, "Content-Type": "application/json" },
            body: JSON.stringify({
              apiVersion: 1, schema: "tcg.price-request/v1", type: "pricing.priceProduct", requestId, target: product,
              options: { includeActive: options.includeActive !== false, includeRecentSales: options.includeRecentSales !== false }
            }),
        ...
        if (!result || typeof result !== "object" || Number(result.apiVersion) !== 1 || result.requestId !== requestId) throw new PricingRestError("pricing service returned an incompatible response", { status: response.status, code: "INVALID_RESPONSE", body: result });
        if (!result.error && (!result.product || result.product.productId !== product.productId)) throw new PricingRestError("pricing service returned a different ProductRef", { status: response.status, code: "PRODUCT_MISMATCH", body: result });
        return result;
      }
    };
  }
